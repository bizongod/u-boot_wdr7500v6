...
#ifdef CONFIG_CUSTOM_MEM_TEST

#define DATA_LINE_NUMBER 		16
#define PATTERN_BIT_NUMBER	32
/**
 *descript : package all pattern
 *例如data1 = 1100,data2 = 0110,
 *测试第一根数据线的pattern[0][0] = 0xFFFF0001, pattern[0][1] = 0x0000FFFE
 *测试第二根数据线的pattern[1][0] = 0xFFFF0002, pattern[1][1] = 0x0000FFFD
 *...
 **/
 void cmtest_pack_pattern(ulong **pattern, char *data1, char *data2, int length)
{
	int i, j, k, l;
	char tmpA, tmpB;
	
	/* 16 根数据线*/
	for (i = 0; i < DATA_LINE_NUMBER; i++){
		l = 0;
		/* 每一根数据线的所有pattern */
		for (j =0; j < length; j++){
			/* 一个pattern的低16bit */
			for (k = 0; k < PATTERN_BIT_NUMBER / 2; k++){
				if (i == k){
					pattern[i][j] =pattern[i][j] | ((data1[l] - '0') << k);
				}
				else
				{
					pattern[i][j] =pattern[i][j] | ((data2[l] - '0') << k);
				}
			}
			...
			/* 一个pattern的高16bit */
			for (k = PATTERN_BIT_NUMBER / 2; k < PATTERN_BIT_NUMBER; k++){
				if ((i + 16) == k){
					pattern[i][j] =pattern[i][j] | ((data1[l] - '0') << k);
				}
				else
...
/*
 *descript:	custom memory test
 *command:	cmtest startAddr endAddr number data1 data2 whichDn
 * parameter: 	number -- 循环序列串的长度
 *			data1    -- 待测数据线的输入序列,例如1100
 *			data2    -- 其他数据线的输入序列,例如0110
 *			whichDn -- 如果有该项参数，表示对某一根数据线进行测试，否则循环对16数据线测试。
 */
int do_mem_cmtest(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	vu_long *addr, *startAddr, *endAddr;
	ulong writeVal, readVal, number, failTimes, iterations;
	ulong  **pattern;
	ulong index, i, whichDn, testFlag;
	...
	startAddr = (ulong *)simple_strtoul(argv[1], NULL, 16);
	endAddr = (ulong *)simple_strtoul(argv[2], NULL, 16);
	...
	/* 分配pattern空间 */
	pattern = (ulong **)malloc(sizeof(ulong *) * DATA_LINE_NUMBER);
	for (i = 0; i < DATA_LINE_NUMBER; i++){
		pattern[i] = (ulong *)malloc(sizeof(ulong) * number);
		memset(pattern[i], 0, sizeof(ulong) * number);
	}
	cmtest_pack_pattern(pattern, argv[4], argv[5], number);
	
	/* 测试某根特定数据线，否则循环测试16根数据线 */
	if (argc == 7){
		whichDn = simple_strtoul(argv[6], NULL, 0);
		testFlag = 1;
	}
	...
	for (; ;){
		...
		for (addr = startAddr; addr < endAddr; addr++){
			readVal = *addr;
			if (readVal != pattern[whichDn][i % number]){
				printf("custom ram test failed, addr = %08lx writeValue = %08lx readValue = %08lx\n",
					addr, pattern[whichDn][i % number], readVal);
				failTimes++;
			}
			i++;
		}
		...
		/* abort test */
		if (ctrlc()){
			printf("custom ram test stop . total run times = %ld, failed times = %ld\n", iterations, failTimes);
			for (index = 0; index < DATA_LINE_NUMBER; index++){
				free(pattern[index]);
			}
			free(pattern);
			return 0;
...
U_BOOT_CMD(
	cmtest, 14, 1, do_mem_cmtest,
	"cmtest - custom RAM test\n",
	"startAddr endAddr dataLength data1 data2\n"
	"custom RAM read/write test\n"
);
