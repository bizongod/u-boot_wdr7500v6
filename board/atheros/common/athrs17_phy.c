...
uint32_t reg_temp, val_temp;
uint16_t reg_counter;

uint16_t rtl_regs[272][3] = { 
   {0, 0x2002, 0x001C},
   {1, 0x13C2, 0x0249},
   {0, 0x1301, 0x00A0},
   {1, 0x13C2, 0x0000},
...
//	reg_temp = rtl_reg_read(0x2002); 
//	printf(" MDIO read reg: %X val: %X \n", 0x2002, reg_temp);

//	rtl_reg_write(0x13C2, 0x0249);

//        reg_temp = rtl_reg_read(0x1301);
//        printf(" MDIO read reg: %X val: %X \n", 0x1301, reg_temp);

//        rtl_reg_write(0x13C2, 0x0249);

	printf("RTL8376S switch config start\n");

	for(reg_counter = 0; reg_counter < 272; reg_counter++)
	{
		if(rtl_regs[reg_counter][0] == 0)	// odczyt
		{

			reg_temp = rtl_reg_read(rtl_regs[reg_counter][1]);
			if (reg_temp != rtl_regs[reg_counter][2])
				printf(" MDIO read reg: %X val: %X ,expected %X \n", rtl_regs[reg_counter][1], reg_temp, rtl_regs[reg_counter][2]);

		} 
		else if (rtl_regs[reg_counter][0] == 1)       // zapis
		{

			rtl_reg_write(rtl_regs[reg_counter][1], rtl_regs[reg_counter][2]);
		}
	...
	printf("RTL8376S switch config done\n");
...
/* For WR1043NDV4 */
#if CONFIG_AP152
        /* Fix Bug98693. Separate WAN/LAN ports, so packets from LAN will not be flooded
         * to WAN port while system rebooting(at uboot stage). */
        //athrs17_reg_write(S17_GLOFW_CTRL1_REG, 0x5f5f5f5f);
        athrs17_reg_write(S17_P0LOOKUP_CTRL_REG, 0x0014007e);
        athrs17_reg_write(S17_P1LOOKUP_CTRL_REG, 0x0014005d);
        athrs17_reg_write(S17_P2LOOKUP_CTRL_REG, 0x0014005b);
        athrs17_reg_write(S17_P3LOOKUP_CTRL_REG, 0x00140057);
        athrs17_reg_write(S17_P4LOOKUP_CTRL_REG, 0x0014004f);
        athrs17_reg_write(S17_P5LOOKUP_CTRL_REG, 0x00140001);
        athrs17_reg_write(S17_P6LOOKUP_CTRL_REG, 0x0014001f);
#endif
...
BOOL
athrs17_phy_setup2(int ethUnit)
{
	int       phyUnit;
	uint16_t  phyHwStatus;
	uint16_t  timeout;
...
athrs17_phy_is_fdx(int ethUnit)
{
	...
	return TRUE;	// SGMII to zawsze fxd, ewentualnie mozna sprawdzic kable

}


int
athrs17_phy_is_fdx2(int ethUnit)
{
	int       phyUnit;
	uint32_t  phyBase;
	uint32_t  phyAddr;
...
athrs17_phy_is_up(int ethUnit)
{
	...
	return 1;	// SGMII zawsze podlaczone, ewentualnie mozna sprawdzic czy kable sa podlaczone
}


int
athrs17_phy_is_up2(int ethUnit)
{
	int           phyUnit;
...
uint32_t rtl_reg_read(uint32_t reg_addr)
{
	...
	phy_reg = 0x1F;
	phy_val = (uint16_t) 0x000E;
	
	phy_reg_write(0, phy_addr, phy_reg, phy_val);

	
	phy_reg = 0x17;
	phy_val = (uint16_t) (reg_addr & 0xFFFF);
	
	phy_reg_write(0, phy_addr, phy_reg, phy_val);	

	
	phy_reg = 0x15;
	phy_val = (uint16_t) 0x0001;
