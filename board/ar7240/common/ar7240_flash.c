...
{
	if (val & 0x1) {
		ar7240_reg_rmw_set(reg, (1 << gpio));
	} else {
		ar7240_reg_rmw_clear(reg, (1 << gpio));
	}
}
...
void ar7240_auf_gpio_init()
{
	...
	ath_gpio_set_val(AR7240_GPIO_OE, FW_BUTTON_GPIO, 1);
	
	/* use wps led as output indicator */
	ath_gpio_set_val(AR7240_GPIO_OE, FW_LED_GPIO, 0);	
}
...
* OTHERS        	: 
******************************************************************************/
void ar7240_fw_led_on()
{	
	ath_gpio_set_val(AR7240_GPIO_OUT, FW_LED_GPIO, FW_LED_ON);
}

void ar7240_fw_led_off()
{
	ath_gpio_set_val(AR7240_GPIO_OUT, FW_LED_GPIO, FW_LED_OFF);
}
#endif
