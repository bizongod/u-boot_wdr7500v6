...
LZMA = $(BUILD_DIR)/../util/lzma/bin/lzma
...
ap123-dsa_config db12x_config			: 	unconfig
	@ >include/config.h
...
ifneq (,$(findstring $(CFG_BOARD_TYPE),cus249 tb753 ap151 tb754)) #{
	@echo '#define CFG_ATH_GMAC_NMACS	2'	>>include/config.h
	@echo '#define CFG_ATH_GE1_IS_CONNECTED 1' 	>>include/config.h
endif #}
ifneq (,$(findstring $(CFG_BOARD_TYPE), ap152 tb755)) #{
...
ifdef PRODUCT_ID
	@echo "#define CONFIG_PRODUCT_ID $(PRODUCT_ID)" >>include/config.h
endif

ifdef LOW_DDR2_FRESH_VAL
	@echo "#define LOW_DDR2_FRESH_VAL 1" >>include/config.h
endif
...
#for uboot nvrammanager
ifeq ($(UBOOT_NVRAMMNGR), y)
	@echo '#define UBOOT_NVRAMMNGR 1' >>include/config.h
ifdef CONFIG_NM_PTN_TABLE_BASE
	@echo "#define NM_PTN_TABLE_BASE $(CONFIG_NM_PTN_TABLE_BASE)" >>include/config.h
endif
endif

	@./mkconfig -a board956x mips mips board956x atheros

ap152_config: unconfig
	@echo '#define CONFIG_ATHEROS		1'	>include/config.h
	@echo '#define CONFIG_MACH_QCA956x	1'	>>include/config.h
	@echo '#define CFG_INIT_STACK_IN_SRAM	1'	>>include/config.h
	@echo '#define CONFIG_'`echo $(CFG_BOARD_TYPE) | tr [a-z] [A-Z] | sed s/-/_/g`'	1' >>include/config.h
